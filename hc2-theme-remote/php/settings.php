<?php
	...
	add_action('admin_notices', 'hcube_notice_unpushed');
	function hcube_notice_unpushed(){
		$unpush = array( 'post' => 0, 'page' => 0, 'web-story' => 0 );
		$postsQuery = get_posts(array('numberposts' => -1,'post_status'=>'publish','post_type'=> array( 'page','post','web-story' )));
		foreach($postsQuery as $post) if( !get_post_meta( $post->ID, 'hc2_post_rendered', 1 ) ) $unpush[$post->post_type]++;
		if( array_sum($unpush) ) echo '<div class="notice notice-warning">'.
				(!$unpush['post']?'':'<p>'.$unpush['post'].' posts are published but not pushed.</p>').
				(!$unpush['page']?'':'<p>'.$unpush['page'].' pages are published but not pushed.</p>').
				(!$unpush['web-story']?'':'<p>'.$unpush['web-story'].' stories are published but not pushed.</p>').
			'</div>';
	}
