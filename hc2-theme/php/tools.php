<?php
	...
	if(is_array($hcube_json_redirects)){
		foreach( $hcube_json_redirects as $redir ) {
			if( ltrim(rtrim($_SERVER['REQUEST_URI'],'/'),'/')==ltrim(rtrim($redir['match'],'/'),'/')){
				if(strpos($redir['redirect'],'http')!==false){
					wp_redirect( $redir['redirect'], '301' );
				}else{
					wp_redirect( get_site_url().$redir['redirect'], '301' );
				}
				exit();
			}
		}
        ...
        ?>
			<div class="wrap">	
				...
				<table class='hcube_settings'>
						...
						<tr><td>
								<table class='hcube_submenu hcube_rend'>

									<tr><td><ul>
									
										<input class="hcube_renderall_button" type="submit" value="Delete All HTML Files"/>
									
									</ul >
									<script>
									
										jQuery('.hcube_renderall_button').on('click',function(e){
											jQuery('.hcube_renderall_button')[0].dataset.loading = 'true';
											jQuery.ajax( { 
												url: '<?php echo get_rest_url(); ?>hc2/v1/renderall/', 
												method: 'POST', 
												beforeSend: function ( xhr ) { xhr.setRequestHeader( 'X-WP-Nonce', '<?php echo wp_create_nonce( "wp_rest" ); ?>' ); },
												data:{ 'postid' : 'all' }
											} ).done( function ( data ) {
												jQuery('.hcube_renderall_button')[0].dataset.loading = 'false';
											} )
										})
