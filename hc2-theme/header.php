...
<head>
	...
	<meta property="og:url" 			content="<?php echo get_page_link( $thispage_id ); ?>"/>
	...
	<script type="application/ld+json">[
		<?php if(get_post_meta( $thispage_id, 'hcube_is_service', true)){ ?>
		{
			...
			"url":"<?php echo get_page_link( $thispage_id ); ?>"
		},
		...
		<?php } else { ?>
		{
			...
			,"image": "<?php echo $used_thumbnail ; ?>"
			,"mainEntityOfPage":"<?php echo get_page_link( $thispage_id ); ?>"
		...
		},
		{
			...
			,"logo" : {"@type" : "ImageObject","url":"<?php if(get_hc2('settings','favicon_id'))echo wp_get_attachment_image_src(get_hc2('settings','favicon_id'))[0]; ?>","width":"60","height":"60"}
			,"image": "<?php echo $used_thumbnail; ?>"
			,"@id": "<?php echo (get_post_meta($thispage_id,'schema_url',1)?get_post_meta($thispage_id,'schema_url',1):get_site_url( )); ?>"
			,"url": "<?php echo (get_post_meta($thispage_id,'schema_url',1)?get_post_meta($thispage_id,'schema_url',1):get_site_url( )); ?>"
			,"description": "<?php echo $filtered_excerpt; ?>"
			...
			<?php } ?>
			,"sameAs":[
				"<?php echo (get_post_meta($thispage_id,'schema_url',1)?get_post_meta($thispage_id,'schema_url',1):get_site_url( )); ?>"
				<?php 
